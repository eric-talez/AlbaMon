...
export function hasCurrentPolicies(user: { policyIdentity?: string | null; termsVersion?: string | null; termsAcceptedAt?: string | null; privacyNoticeVersion?: string | null; privacyNoticeAcknowledgedAt?: string | null }, expectedIdentity: string) {
  return user.policyIdentity === expectedIdentity && !!expectedIdentity && user.termsVersion === TERMS_VERSION && !!user.termsAcceptedAt &&
