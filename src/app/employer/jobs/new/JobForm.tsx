...
export function JobForm({ companies }: { companies: EmployerCompany[] }) {
  ...
  return (
    <form action={formAction} className="mt-6 space-y-5 rounded-xl border border-border bg-surface p-5">
      ...
      <label className="block text-sm font-medium" htmlFor="job-companyId">
        회사
      ...
      <label className="block text-sm font-medium" htmlFor="job-title">
        공고 제목
        <input className={inputClass} id="job-title" name="title" required maxLength={120} />
      </label>
      <div className="grid gap-4 sm:grid-cols-2">
        <label className="block text-sm font-medium" htmlFor="job-category">
          직종
          <select className={inputClass} id="job-category" name="category" required defaultValue="">
        ...
        <label className="block text-sm font-medium" htmlFor="job-jobType">
          고용 형태
          <select className={inputClass} id="job-jobType" name="jobType" required defaultValue="">
            <option value="" disabled>선택</option>
        ...
        <label className="flex items-center gap-2 self-end pb-2 text-sm font-medium" htmlFor="job-tipsAvailable">
          <input type="checkbox" id="job-tipsAvailable" name="tipsAvailable" /> 팁 별도 제공
        </label>
        <label className="block text-sm font-medium" htmlFor="job-scheduleDays">근무 요일<input className={inputClass} id="job-scheduleDays" name="scheduleDays" required maxLength={200} placeholder="예: 월–금" /></label>
        <label className="block text-sm font-medium" htmlFor="job-scheduleTimeRange">근무 시간<input className={inputClass} id="job-scheduleTimeRange" name="scheduleTimeRange" required maxLength={200} placeholder="예: 09:00–17:00" /></label>
      </div>
      <label className="block text-sm font-medium" htmlFor="job-languageRequirement">
        언어 요건
        <select className={inputClass} id="job-languageRequirement" name="languageRequirement" required defaultValue="">
          <option value="" disabled>선택</option>
          {LANGUAGE_REQUIREMENTS.map((value) => <option key={value} value={value}>{LANGUAGE_REQUIREMENT_LABELS[value]}</option>)}
        </select>
      </label>
      <label className="block text-sm font-medium" htmlFor="job-description">상세 설명<textarea className={inputClass} id="job-description" name="description" required rows={8} maxLength={5_000} /></label>
      <label className="block text-sm font-medium" htmlFor="job-responsibilities">담당 업무 <span className="font-normal text-muted">(선택, 한 줄에 하나)</span><textarea className={inputClass} id="job-responsibilities" name="responsibilities" rows={4} /></label>
      <label className="block text-sm font-medium" htmlFor="job-requirements">자격 요건 <span className="font-normal text-muted">(선택, 한 줄에 하나)</span><textarea className={inputClass} id="job-requirements" name="requirements" rows={4} /></label>
      <label className="block text-sm font-medium" htmlFor="job-benefits">복리후생 <span className="font-normal text-muted">(선택, 한 줄에 하나)</span><textarea className={inputClass} id="job-benefits" name="benefits" rows={4} /></label>
