...
export const metadata: Metadata = { title: "지원 메시지" };
...
export default async function SeekerApplicationMessagesPage({
  ...
  return (
    <main className="mx-auto w-full max-w-3xl flex-1 px-4 py-10">
      <Link href="/dashboard/applications" className="text-sm font-medium text-brand hover:underline">← 지원 내역</Link>
      <p className="mt-6 text-xs font-medium text-brand">K-Work US 지원자 메시지</p>
      <h1 className="mt-1 text-2xl font-bold">지원 메시지</h1>
      {result.status !== "ok" ? (
