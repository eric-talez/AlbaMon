...
if (process.env.EMAIL_PROVIDER !== "resend" || process.env.EMAIL_NOTIFICATIONS_ENABLED !== "true" ||
...
if (process.env.CRON_SECRET.trim().length < 16) throw new Error("CRON_SECRET requires at least 16 characters");
const mailbox = /^[^\s<>@*,;]+@[^\s<>@*,;]+\.[^\s<>@*,;]+$/;
...
if (process.env.EMAIL_ENVIRONMENT === "staging") {
  const recipients = (process.env.EMAIL_STAGING_ALLOWLIST ?? "").split(",").map(value => value.trim());
