...
async function setup(context?: BrowserContext) {
  ...
  if ((await owner.rpc("acknowledge_policies", { terms: "ca-launch-v1", agree_terms: true, privacy_notice: "ca-launch-v1", confirm_privacy_notice: true, publication_identity: policyAcceptanceIdentity() })).error) throw new Error("Fixture self acknowledgement failed");
  ...
  return {service,owner,ownerId,jobId,companyId,async cleanup(){
    const notifications=await service.from("notification_outbox").delete().in("entity_id",[jobId,ownerId]);
    if(notifications.error) throw new Error("Lifecycle notification cleanup failed");
    await service.from("audit_logs").delete().in("entity_id",[jobId,ownerId]);
