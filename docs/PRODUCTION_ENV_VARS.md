...
| `SUPABASE_SERVICE_ROLE_KEY` | **server-only** | Vercel; same Supabase page | `<service-role-key>` | Bypasses RLS entirely. Its only app consumer is the durable rate limiter (Slice 28): the client in `src/lib/supabase/service.ts` calls the private `consume_rate_limit` RPC and nothing else — never OTP send/verify and never a business mutation. `/api/health` reports the key's *presence* only. Keep any future usage restricted to trusted server flows. |
| `RATE_LIMIT_HMAC_SECRET` | **server-only** | Vercel (Production + Preview scope) | `generate-with-openssl-rand-hex-32` | HMAC key for the durable rate limiter (Slice 28). Must be exactly **64 hex chars → 32 bytes** (`openssl rand -hex 32`); it is decoded to the 32-byte key and used to hash phone/IP/user subjects before they reach the DB (raw identifiers are never stored). Validated at call time in `src/lib/rate-limit/keys.ts` — a missing/placeholder/non-hex value counts as *unconfigured*, and in production/preview the rate-limited actions (phone OTP, high-risk writes) **fail closed** (deny). No default is shipped. Rotating it starts a fresh bucket namespace (old counters become unreachable and expire). |
